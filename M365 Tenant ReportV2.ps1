...
$MSPLogo    = "MSP Logo URL"
$FavIcon    = "Favorite Icon URL"
...
try {
[array]$AuditRecords = Get-MgBetaAuditLogSignIn -Top 50000 `
...
[int]$MFAUsers = 0
    ...
    Write-Host "Falling back to AzureAD module..."
   
    # Check if AzureADPreview module is installed in PowerShell 5.1
    $azureADPreviewInstalled = Get-Module -ListAvailable -Name AzureADPreview -ErrorAction SilentlyContinue

    # If AzureADPreview module is not installed, install it
    if (-not $azureADPreviewInstalled) {
    Write-Host "AzureADPreview module not found in PowerShell 5.1. Installing..."
    Start-Process -FilePath "powershell.exe" -ArgumentList "-Command Install-Module -Name AzureADPreview -Force -AllowClobber" -Wait}
    ...
    ForEach ($User in $Users) {
        ...
        $mfaStatus = if ($authMethods.Count -gt 1) { 
            'Enabled' 
        } else { 
            'Not Enabled' 
        }
            ...
            New-HTMLSection -CanCollapse -HeaderText 'MFA User Status Chart' -HeaderBackGroundColor '#0b224c' {           
                New-HTMLChart {
                    ...
                    if ($MFAEnforcedUsersCount -gt 0 -or $MFANonEnabledUsersCount -gt 0 -or $MFAEnabledUsersCount -gt 0) {
                        ...
                        New-ChartPie -Name 'Number of accounts Enabled for MFA' -Value $MFAEnableddUsers.count -Color '#6dbf88'    
                    }        
                }       
            }
        }
